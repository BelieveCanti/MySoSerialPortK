...
public abstract class SerialHelper {
	...
	public SerialHelper(String port, int baudRate,int stopBits,int DataBits,int Parity) {
		mPort = port;
		mBaudRate = baudRate;
		mStopBits = stopBits;
		mDataBits = DataBits;
	 ...
	 * 	 * @param baudrate  BAUDRATE波特率
	 * 	 * @param stopBits  STOPB停止位
	 * 	 * @param dataBits  DATAB数据位
	 * 	 * @param parity    PARITY校验位
	 * 	 * @param flowCon   FLOWCON流控
	 * 	 * @param flags    O_RDWR  读写方式打开 | O_NOCTTY  不允许进程管理串口 | O_NDELAY   非阻塞
	 */
	public void open() throws SecurityException, IOException, InvalidParameterException {
		mSerialPort = new SerialPort(new File(mPort), mBaudRate, mStopBits,mDataBits, mParity,flowCon,flags);
//		Log.i("TAG", "open: " +  mBaudRate+ mStopBits+mDataBits+ mParity+flowCon+flags);
